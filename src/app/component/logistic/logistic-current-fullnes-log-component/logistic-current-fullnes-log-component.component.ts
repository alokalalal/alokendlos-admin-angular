...
@Component({
  selector: 'app-logistic-current-fullnes-log-component',
  templateUrl: './logistic-current-fullnes-log-component.component.html',
  styleUrls: ['./logistic-current-fullnes-log-component.component.css']
})
export class LogisticCurrentFullnesLogComponentComponent implements OnInit {
  ...
  public listContainer: ListContainer = {
      pageTitle: "Current Fullness Logs",
      accessRightsJson: {
          ...
          if (value.isApplied) {
              switch (key) {
                ...
                case "pickupRouteView":               
                    ...
                    break;
                default:
                    searchBody['machineId'] = value.appliedValue
                    break;
              }
          }
      });
  ...
  exportList(searchBody: any) {
       this.listContainerBlockUi.start();
      this.pickupRouteService.doExportCurrentfullnessLog(this.orderType.key, this.orderParam.key, searchBody).subscribe((response: CommonViewResponse<FileView>) => {
          if (response != undefined && response.code >= 1000 && response.code < 2000) {
