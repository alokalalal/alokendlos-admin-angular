...
import { PlcConfigurationView } from '../entities/plc-configuration-view';

@Injectable({
    providedIn: 'root'
})
export class PLCConfigurationService {
    appUrl = AppUrl;
    apiUrl = ApiConfig;
    searchFilter: any = {};
    ...
    doSearch(start: number | string, recordSize: number | string, orderType: number | string, orderParam: number | string, searchBody: any): Observable<any> {
      return this.http.post<CommonListResponse<PlcConfigurationView>>(
        AppUrl.BASE_URL + Apiurl.PRIVATE_PLC_CONFIGURATION_URL + Apiurl.SEARCH +
    ...
    doExport(orderType: number | string, orderParam: number | string, searchBody: any): Observable<any> {
      return this.http.post<CommonListResponse<PlcConfigurationView>>(
        AppUrl.BASE_URL + Apiurl.PRIVATE_PLC_CONFIGURATION_URL + "/export" +
        '?' + ApiUrlParameter.ORDER_TYPE_URL + (orderType ? orderType : "") +
        '&' + ApiUrlParameter.ORDER_PARAM_URL + (orderParam ? orderParam : "")
        , searchBody);
    }
    ...
    doDownload(fileId: number | string){
      return this.http.get<CommonViewResponse<PlcConfigurationView>>(this.appUrl.BASE_URL + '/private/file/download-excel-file?fileid=' + fileId);
    }

    doGetOrderParameter(): Promise<CommonListResponse<KeyValueView>> {
      return new Promise(resolve => {
        this.http.get<CommonListResponse<KeyValueView>>(AppUrl.BASE_URL + Apiurl.PRIVATE_PLC_CONFIGURATION_URL + Apiurl.GET_ORDER_PARAMETER)
